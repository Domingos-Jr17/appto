/**
 * Minimal test: Nemotron raw text response
 * Usage: node scripts/test-nemotron-raw.mjs
 */
...
async function testNemotron() {
  console.log("=== Testing Nemotron 3 Super (Raw) ===\n");
...
- JSON estritamente válido`;
  ...
  try {
    ...
    console.log(`Content words: ${content.split(/\s+/).filter(Boolean).length}`);
