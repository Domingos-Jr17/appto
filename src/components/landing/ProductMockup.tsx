...
export function ProductMockup() {
  const t = useTranslations("landing.productMockup");
  const sidebarItems = [
    { icon: FolderOpen, label: t("sidebar.projects"), active: true },
    { icon: PenTool, label: t("sidebar.editor") },
    { icon: Layout, label: t("sidebar.structure") },
    { icon: BookMarked, label: t("sidebar.references") },
  ...
  return (
    <div className="relative">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/50 bg-muted/30">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              <span>{t("window.remainingWorks")}</span>
            </motion.div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              >
                <h3 className="text-lg font-semibold text-foreground mb-1">
                  {t("document.sectionTitle")}
                </h3>
                <p className="text-xs text-muted-foreground">
                  {t("document.sectionMeta")}
                </p>
              </motion.div>
              ...
              >
                <div className="p-3 rounded-xl bg-muted/40 border border-border/30">
                  <p className="text-sm text-foreground/90 leading-relaxed">
                    {t("document.paragraph.beforeHighlight")} <span className="bg-primary/20 text-primary font-medium px-1 rounded">{t("document.paragraph.highlight")}</span>{t("document.paragraph.afterHighlight")}
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
                    ...
                    <span>{t("document.aiSuggestionLabel")}</span>
                  </div>
                  <p className="text-sm text-foreground/80">
                    {t("document.aiSuggestion")}
                  </p>
                  <div className="flex gap-2 mt-3">
                    ...
                    >
                      ...
                      />
                      <span className="relative">{t("actions.accept")}</span>
                    </motion.button>
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
      ...
      >
        ...
        >
          <FloatingBadge icon={Languages} label={t("badges.locale")} />
        </motion.div>
      </motion.div>
      ...
      >
        ...
        >
          <FloatingBadge icon={BookMarked} label={t("badges.abnt")} />
        </motion.div>
      </motion.div>
      ...
      >
        ...
        >
          <FloatingBadge icon={FileDown} label={t("badges.docx")} />
        </motion.div>
      </motion.div>
      ...
      {isFeaturePublic("realTimeStreaming") ? (
        ...
        >
          ...
          >
            <FloatingBadge icon={Zap} label={t("badges.streaming")} />
          </motion.div>
        </motion.div>
      ) : null}
    </div>
