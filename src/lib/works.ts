...
import type { WorkspaceData, WorkSection, SectionStatus } from "@/types/workspace";

function deriveStatus(content: string | null, wordCount: number, isGenerating: boolean): SectionStatus {
  if (isGenerating && !content) return "generating";
  if (wordCount > 0 || (content && content.trim().length > 0)) return "done";
  return "pending";
}

export async function getWork(projectId: string): Promise<WorkspaceData | null> {
  ...
  const generationStatus = liveGeneration?.status || project.brief?.generationStatus || "BRIEFING";
  const generationProgress = liveGeneration?.progress ?? (generationStatus === "READY" ? 100 : 0);
  ...
  return {
    ...
    brief: {
      ...
      institution: project.brief?.institutionName ?? undefined,
      course: project.brief?.courseName ?? undefined,
      subject: project.brief?.subjectName ?? undefined,
      advisor: project.brief?.advisorName ?? undefined,
